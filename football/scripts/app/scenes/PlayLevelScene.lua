...
function PlayLevelScene:ctor(levelIndex)
    ...
    -- local texture = CCTextureCache:sharedTextureCache():addImage("prop_click.png")
    -- if texture == nil then
    --     print("dfdsfdf")
    -- end
    -- local num_tex = 5
    ...
    -- local table_frames = {}
    -- for i=1,5 do
    --     table_frames[i] = CCSpriteFrame:createWithTexture(texture, CCRectMake((i-1)*tex_width, 0, tex_width, tex_height))
    -- end
    -- local prop_sp = CCSprite:create("prop_click.png",CCRectMake(0, 0, tex_width, tex_height))
    -- -- :setPosition(ccp(display.cx,display.cy))
    -- prop_sp:setPosition(ccp(display.cx,display.cy))
    -- self:addChild(prop_sp,100)
    -- if table_frames[1] == nil then
    --     print("dfdsfdf222")
    -- end
    -- if prop_sp == nil then
    --     print("dfdsfdf111")
    -- end
    -- local animation = display.newAnimation(table_frames, 1 / 5)
    -- local action = CCAnimate:create(animation)
    -- local rep = CCRepeatForever:create(action)
    -- prop_sp:runAction(rep)
    ...
    -- self.img_lf = display.newSprite("lf.png")
    --     :align(display.CENTER,display.left + 250,display.bottom + 130)
    --     :addTo(self)
    -- self.img_lf:setTouchEnabled(true)
    -- self.img_lf:addNodeEventListener(cc.NODE_TOUCH_EVENT,function(event)
    ...
    local menu = ui.newMenu({backButton,restartButton,shopButton})
    self:addChild(menu)
