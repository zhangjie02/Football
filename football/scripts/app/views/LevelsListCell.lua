...
function LevelsListCell:ctor(size, beginLevelIndex, endLevelIndex, rows, cols)
    ...
    local batch = display.newBatchNode("btn_level.png")
    self:addChild(batch)
    ...
    for row = 1, rows do
        ...
        for column = 1, cols do
            local icon = display.newSprite("btn_level.png", x, y)
            batch:addChild(icon)
            icon.levelIndex = levelIndex
            self.buttons[#self.buttons + 1] = icon

            if levelIndex <= data.record_level_index then
                ...
                self.batch = display.newBatchNode("btn_lock.png")
                self:addChild(self.batch)
                local img_lock = display.newSprite("btn_lock.png",x,y-4)
                self.batch:addChild(img_lock)
            end
...
function LevelsListCell:onTap(x, y)
    local button = self:checkButton(x, y)
    if button and button.levelIndex <= data.record_level_index then
        data.cur_level_index = button.levelIndex
